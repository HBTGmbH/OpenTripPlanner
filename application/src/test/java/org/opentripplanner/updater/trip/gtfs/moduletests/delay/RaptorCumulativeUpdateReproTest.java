...
/**
 * Reproduction probe for PR #7744: does a realtime update applied on one service date survive
 * in the realtime RaptorTransitData after a *later* commit that only touches a *different*
 * service date?
 */
class RaptorCumulativeUpdateReproTest implements RealtimeTestConstants {
  ...
  void updateOnDate1SurvivesCommitTouchingOnlyDate2() {
    ...
    // DATE_2 is now updated.
    assertThat(env.raptorData(DATE_2).summarizePatterns()).containsExactly("F:Pattern1[U]");

    // THE KEY ASSERTION: DATE_1's earlier realtime update must still be present in the
    // realtime RaptorTransitData of the latest snapshot.
    assertThat(env.raptorData(DATE_1).summarizePatterns()).containsExactly("F:Pattern1[U]");
  }
}
